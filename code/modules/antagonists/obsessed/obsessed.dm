...
/datum/objective/assassinate/jealous/find_target(dupe_search_range, list/blacklist)
	...
	for(var/datum/mind/crewmember as anything in get_crewmember_minds())
		if(crewmember == obsessed_target || crewmember.has_antag_datum(/datum/antagonist/obsessed)  || crewmember.has_antag_datum(/datum/antagonist/contract_killer))
			continue // the jealousy target has to have a job, and not be the obsession or obsessed. ORBSTATION EDIT: or contract killer
